...
3. [Your First Mission (Full-team)](#3-your-first-mission-full-team)
...
7. [Connecting Linear (Optional)](#7-connecting-linear-optional)
8. [MCP Server Recommendations](#8-mcp-server-recommendations)
9. [Resuming a Session](#9-resuming-a-session)
10. [Hooks -- Mechanical Enforcement](#10-hooks----mechanical-enforcement)
...
- [Linear](https://linear.app) API key -- for collaboration visibility, issue tracking, and human intervention during agent work. Geas works without Linear, but the experience is richer with it.
- Python 3.10+ -- required if you want Linear integration (the Linear CLI wrapper is written in Python) and for the hook system that enforces pipeline integrity.
...
## 3. Your First Mission (Full-team)

Full-team mode is for building a new product from scratch. It runs through four phases: Genesis, MVP Build, Polish, and Evolution.
...
That is all you need. Compass detects that this is a new product and activates Full-team mode automatically. Here is what happens step by step.
...
If you want Linear integration, setup will walk you through connecting your API key and selecting a team. If you decline, the team uses `.geas/` as the sole state store.
...
Each answer is scored across five dimensions (Clarity, Scope, Users, Constraints, Acceptance) on a 0-20 scale. Full-team mode requires a readiness score of at least 60 out of 100 before proceeding.
...
|-------------|---------------|
| New product or broad mission | Full-team |
| Adding a feature to an existing project | Sprint |
| Decision-only discussion, no code | Debate |

You can also force a mode directly by invoking `/geas:full-team`, `/geas:sprint`, or `/geas:debate`.
...
If Linear is connected, each task becomes an issue.
...
### How Sprint Differs from Full-team

| Aspect | Full-team | Sprint |
|--------|-----------|--------|
...
7. **Evidence Gate** -- Same 3-tier gate as Full-team (mechanical, semantic, product).
...
Debate produces no code. The DecisionRecord becomes input for future Full-team or Sprint runs.
...
**`packets/{task-id}/*.md`** -- ContextPackets. Focused, role-specific briefings generated by Compass before dispatching each agent. Rather than making agents read entire Linear threads or scan the whole codebase, packets contain only the context that specific agent needs.

**`state/run.json`** -- The current execution state. Contains the mission name, current mode (full-team/sprint/debate), current phase (genesis/mvp/polish/evolve), current task ID, list of completed tasks, and overall status. This is what allows session resumption.
...
## 7. Connecting Linear (Optional)

Linear is the current collaboration surface for Geas. It provides a place where you can watch agent activity, read their reasoning, and intervene with comments that agents will see.

### Setting Up

During the setup phase, Compass will ask:

> "Do you want to connect Linear for issue tracking?"

If you say yes:

1. **API Key.** You need a Linear API key. Get one from [linear.app/settings/api](https://linear.app/settings/api). The key starts with `lin_api_`.

2. **Storage.** The key is stored in a `.env` file at the project root (or detected from a parent directory's `.env` if one already exists).

3. **Team Selection.** If you have multiple Linear teams, you will be asked to choose one.

4. **Workspace Setup.** Geas creates labels (feature, bug, design-spec, architecture, etc.) and verifies workflow states (Backlog, Todo, In Progress, In Review, Testing, Done, Canceled, Waiting) in your Linear workspace.

5. **Configuration.** Linear settings are saved to `.geas/config.json`.

### What Linear Provides

- **Issue Tracking.** Each TaskContract becomes a Linear issue. Status transitions (In Progress, In Review, Testing, Done) are managed by Compass.

- **Agent Comments.** Agents post comments on their assigned issues using a standard format: `[AgentName] summary content`. You can follow the conversation in real time.

- **Human Intervention.** Your comments on Linear issues are included in agent ContextPackets. If you write "Use bar charts instead of pie charts," the next agent to work on that task will see your feedback.

Example Linear thread:
...
If you skip Linear, everything still works. The difference is:

- No real-time comment stream to follow. You inspect `.geas/` for progress.
- No human intervention via comments. You interact directly in the Claude Code session.
- Agent evidence is still written to `.geas/evidence/` the same way.

---

## 8. MCP Server Recommendations
...
## 10. Hooks -- Mechanical Enforcement
...
- **"LINEAR_API_KEY not set"**: No `.env` file found with the key. Run setup again or manually create `.env` with `LINEAR_API_KEY=lin_api_...`.
- **Auth errors**: The key may be expired or invalid. Generate a new one at [linear.app/settings/api](https://linear.app/settings/api).
- **Key in parent directory**: The Linear CLI uses `dotenv` which walks up the directory tree. A `.env` in a parent directory will be found automatically.
