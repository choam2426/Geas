...
| 스킬 | 설명 | 호출 방식 | 입력 | 출력 |
|-------|------|-----------|------|------|
| [briefing](#briefing) | 제품 건강 상태에 대한 Nova의 구조화된 상태 보고 | 마일스톤, 페이즈 전환 시 또는 요청 시 | 실행 상태, Linear 이슈, 이전 브리핑 | 상태 보고서 (Linear 코멘트 + 콘솔) |
| [cleanup](#cleanup) | AI 슬롭, 데드 코드, 컨벤션 이탈을 탐지하는 엔트로피 스캔 | MVP 이후 또는 Evolution 중 | 소스 파일, conventions.md | Linear에 기술 부채 이슈 |
| [coding-conventions](#coding-conventions) | AI 스타트업 워크스페이스를 위한 범용 코딩 표준 -- 스택 무관 | 구현 시 에이전트가 참조 | N/A (참조 문서) | N/A (표준 정의) |
| [ledger-query](#ledger-query) | `.geas/ledger/events.jsonl`에 대한 읽기 전용 검색 | 진단, 상태 확인 또는 히스토리 조회 시 | 쿼리 타입 + 선택적 필터 | 서식화된 마크다운 테이블 |
...
| [run-summary](#run-summary) | 세션 종료 요약 생성 -- 결정사항, 완료된 이슈, 에이전트 통계, verify-fix 루프. Linear Document와 콘솔에 게시 | 세션 종료 시, 핸드오프 시 또는 요청 시 | 실행 상태, 에이전트 로그, Linear 이슈 | Linear Document + 콘솔 출력 |
...
| [write-prd](#write-prd) | 기능 아이디어 또는 미션으로부터 PRD 작성 | 요청 시 또는 Genesis 중 | 기능 아이디어, 문제 정의, 미션 | 마크다운 PRD |
| [write-stories](#write-stories) | 기능이나 미션을 수락 기준이 포함된 사용자 스토리로 분해 | 요청 시 또는 계획 단계 중 | 기능 설명 또는 미션 정의 | 마크다운 사용자 스토리 |
...
| 페이즈 | 주요 활동 |
|--------|----------|
| **Genesis** | Seed 확인, Linear 부트스트랩, Nova 비전, Forge 아키텍처, vote round (Critic 필수 참여), TaskContract 컴파일, MCP 서버 추천 |
...
| **Evolution** | seed 범위 내 개선, Nova 최종 브리핑, Keeper 릴리스 관리 |
...
파이프라인: Compile TaskContract -> Design (Palette) -> Tech Guide (Forge) -> Implementation (worktree) -> Code Review (Forge) -> Testing (Sentinel) -> Evidence Gate -> Nova Product Review -> Ship Gate -> Retrospective (Scrum) -> Resolve (Keeper 커밋).
...
| **호출 시점** | 모든 세션 종료 시, 세션 핸드오프 전, 또는 명시적 요청 시. |
...
| **호출 시점** | 요청 시 또는 Genesis 페이즈 계획 중. |
| **입력** | 기능 아이디어, 문제 정의 또는 미션 (인자로 전달). |
| **출력** | 마크다운 PRD (Problem, Objective, Target Users, Scope, User Flows, Requirements, Success Metrics, Open Questions). |
...
| **호출 시점** | 요청 시 또는 계획 단계 중. |
| **입력** | 기능 설명, 미션 정의, 또는 해결할 문제 (인자로 전달). |
| **출력** | 마크다운 사용자 스토리. 각 스토리에 "As a / I want to / So that" 형식, 수락 기준 체크리스트, 우선순위 (P0/P1/P2), 규모 추정치 (S/M/L) 포함. |
