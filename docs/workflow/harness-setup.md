...
`Harness Setup`은 Agent가 `Work`를 실행, 측정, 관찰, 검토, 복원 가능한 방식으로 수행하기 위해 필요한 준비를 구성하는 조건부 단계다.

Agent는 `Work`의 결과 품질, 실행 가능성, 측정 가능성, 위험 통제, User 판단 비용을 의미 있게 바꾸는 준비를 `Harness Setup`으로 다룬다.

`Harness Setup`은 `Work Frame`, `Task Contract`, `Mission Brief`에서 정한 기준에 연결된다.

## 수행 조건

Agent는 `Work Frame`, `Task Contract`, `Mission Brief`를 정리할 때 별도 harness가 필요한지 판단하고, 필요한 경우 어떤 수준으로 구성할지 제안한다.

|질문|드러낼 것|
|---|---|
|현재 맥락만으로 실행 품질이 충분한가?|추가 context, 문서, 코드맵, 외부 자료 필요 여부|
|실행을 재현할 수 있는가?|환경, 명령, fixture, seed data, mock, replay 필요 여부|
...
|`Task`|필요한 harness를 `Task Contract`의 `Starting Context`, `Execution`, `Verification Strategy`, `Review And Challenge Focus`에 연결한다.|
|`Mission`|공유 harness를 `Mission Brief`의 `Key Context`, `Task Structure`, `Validation And Review Strategy`, `Mission Synthesis Plan`에 연결하고, 개별 harness는 각 `Task Contract`에서 고정한다.|

## User 판단 지점

다음 경우 Agent는 harness 준비를 User 판단 지점으로 올린다.
...
|harness 준비가 작업량과 일정에 영향을 준다.|준비 범위, 깊이, 생략 가능한 항목|
...
작업 품질, 실행 가능성, 측정 가능성, 위험 통제, User 판단 비용을 의미 있게 바꾸지 않는 준비는 생략한다.

단순 실행보다 준비 비용이 더 크고 결과 판단을 개선하지 않는 경우 Agent는 생략 이유를 현재 `Work`에 맞는 위치에 남긴다.

권한, 보안, 비용, 외부 연결, 장기 유지 부담이 생기는 harness는 Agent가 임의로 고정하지 않고 User 판단 지점으로 올린다.
