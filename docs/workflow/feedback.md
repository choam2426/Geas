...
User feedback은 `Work Frame`, `Task Contract`, `Mission Brief`, `Outcome`, `Evidence` 중 무엇을 갱신해야 하는지 판단하는 입력이다.
...
|Affected Basis|feedback이 `Work Frame`, `Task Contract`, `Mission Brief`, `Outcome`, `Evidence` 중 어디에 영향을 주는지 잡는다.|
...
|---|---|
|`Outcome` 품질 문제|같은 `Task Contract` 안에서 `Rework`한다.|
|`Evidence` 부족|`Evidence`를 보완하거나 verification, review, challenge를 강화한다.|
|`Task Contract` 기준 변경|현재 `Task`를 `Cancel`하고 새 `Work Frame`을 만든다.|
