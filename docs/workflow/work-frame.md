...
`Work Frame`은 User와 Agent가 같은 이슈를 보고 있는지 확인하게 하고, Agent가 이 `Work`를 `Direct Work`, `Task`, `Mission` 중 어떤 방식으로 다룰지 제안하게 한다. 또한 별도 [Harness Setup](./harness-setup.md)이 필요한지 판단할 기준을 제공한다.
