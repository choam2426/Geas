...
`Task`는 필요한 경우 [Harness Setup](./harness-setup.md) 결과를 `Starting Context`, `Guardrails`, `Verification Strategy`, `Review And Challenge Focus`에 반영한다.
...
|필요한 작업 준비가 다르다.|style, conventions, environment, fixture, benchmark, observability, safety scan, eval suite처럼 준비와 판단 표면이 달라진다.|
...
|`Execute`|Agent가 `Task Contract` 기준 안에서 실제 작업을 수행한다. 기준을 바꾸지 않는 세부 runtime 준비는 이 단계에서 수행한다.|
...
|Starting Context|수용된 선행 결과, 기준 산출물, 필요한 입력, 전제, 먼저 읽거나 확인할 파일, 문서, 환경 정보, 테스트, 실행 흐름을 고정한다.|
|Execution|Agent가 실제로 수행할 작업을 고정한다.|
|Deliverable|`Task`가 남길 결과물을 고정한다.|
|Acceptance Criteria|User가 받아들일 기준을 고정한다.|
|Guardrails|지켜야 할 제약, 보존할 영역, 유지해야 할 style, conventions, 위험 통제 기준을 고정한다.|
|Impact Surface|변경이나 산출물이 영향을 줄 수 있는 파일, 문서, 흐름, 의존 관계를 고정한다.|
|Verification Strategy|어떤 테스트, 실행 확인, 수동 확인, regression 확인과 필요한 확인 표면을 사용할지 고정한다.|
