...
`Task`는 필요한 경우 [Harness Setup](./harness-setup.md)을 실행 기준 안에 포함한다.
...
|필요한 harness가 다르다.|context, environment, fixture, benchmark, observability, safety scan, eval suite처럼 준비와 판단 표면이 달라진다.|
...
flowchart TD
  ...
  contractAccept --> harness["Harness Setup (if needed)"]
...
|`Harness Setup`|필요한 context, tool, script, environment, fixture, metric, observability, safety, evaluation surface를 실행 전에 구성한다.|
...
|`Change Trigger`|목표, 경계, 산출물, 수용 기준, harness 준비, 확인 방법, 영향 범위, 위험 수준의 변화를 드러낸다.|
...
|실행 전에 어떤 harness를 준비하는가?|`Harness Setup Plan`|
...
|Harness Setup Plan|실행 전에 구성할 context, tool, script, environment, fixture, metric, observability, safety, evaluation surface와 User 판단이 필요한 준비를 고정한다.|
...
`Accepted Decisions`는 방향 선택, tradeoff, 조건, 위험 수용처럼 실행 기준을 바꾸는 User 결정을 고정한다. `Starting Context`는 실행 전에 이해해야 할 기준과 입력이다. `Harness Setup Plan`은 실행, 측정, 관찰, 검토, 복원을 위해 준비할 표면을 정한다. `Impact Surface`는 변경이나 산출물이 닿을 수 있어 확인해야 할 표면이다. `Verification Strategy`는 실제 확인 행위를 정한다. `Review And Challenge Focus`는 분리된 context에서 볼 관점을 정하고, `Evidence Plan`은 User 판단을 위해 남길 근거, 판단 표면, 한계를 정한다.

`Task`는 실행 전에 `Task Contract`를 User가 받아들여야 한다. `Task`의 `Goal`, `Boundary`, `Execution`, `Harness Setup Plan`, `Deliverable`, `Acceptance Criteria`, 확인 방법, 영향 범위, 위험 수용 조건이 바뀌면 현재 `Task`를 `Cancel`하고 새 `Work Frame`을 작성한다.
...
`Rework`는 같은 `Task Contract` 안에서만 사용한다. `Goal`, `Boundary`, `Execution`, `Harness Setup Plan`, `Deliverable`, `Acceptance Criteria`, 위험 수용 조건이 바뀌면 현재 `Task`를 `Cancel`하고 새 `Work Frame`을 작성한다.
